...
try:
    ...
    for i in range(dimension - 1):
        sin_inter_in = m.addVar(vtype=GRB.CONTINUOUS, name="sin_inter_in" + str(i))
        sin_inter_out = m.addVar(vtype=GRB.CONTINUOUS, name="sin_inter_out" + str(i))
        m.addConstr(sin_inter_in == np.pi * w[i] + 1, "sin_inter_in" + str(i))
        m.addGenConstrSin(sin_inter_in, sin_inter_out, "sin_inter_out" + str(i))

        term_inter_1 = m.addVar(vtype=GRB.CONTINUOUS, name="term_inter_1" + str(i))
        term_inter_2 = m.addVar(vtype=GRB.CONTINUOUS, name="term_inter_2" + str(i))
        m.addConstr(term_inter_1 == (w[i] - 1) ** 2, "term_inter_1" + str(i))
        m.addConstr(term_inter_2 == 1 + 10 * sin_inter_out ** 2, "term_inter_2" + str(i))
        summation_item = m.addVar(vtype=GRB.CONTINUOUS, name="summatino_item" + str(i))
        m.addConstr(summation_item == term_inter_1 * term_inter_2, "summatino_item" + str(i))
