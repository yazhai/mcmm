...
class Levy(TestFunction):
    ...
    def encode_to_gurobi(self, m: gp.Model = None, x: List[gp.Var] = None):
        ...
        for i in range(self.dims - 1):
            sin_inter_in = m.addVar(
                vtype=GRB.CONTINUOUS, name="var_sin_inter_in" + str(i)
            )
            sin_inter_out = m.addVar(
                vtype=GRB.CONTINUOUS, name="var_sin_inter_out" + str(i)
            )
            ...
            term_inter_1 = m.addVar(
                vtype=GRB.CONTINUOUS, name="var_term_inter_1" + str(i)
            )
            term_inter_2 = m.addVar(
                vtype=GRB.CONTINUOUS, name="var_term_inter_2" + str(i)
            )
            ...
            summation_item = m.addVar(
                vtype=GRB.CONTINUOUS, name="var_summation_item" + str(i)
            )
            m.addConstr(
...
class Michalewicz(TestFunction):
    ...
    def encode_to_gurobi(self, m: gp.Model = None, x: List[gp.Var] = None):
        ...
        for i in range(self.dims):
            ...
            m.addGenConstrPow(pow_in, pow_out, 2 * self.m, "pow_out" + str(i))
