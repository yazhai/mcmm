...
def partition_space(
    ...
    xs: npt.ArrayLike, 
    dim: int,
    ignore_dist: float=0) -> typing.List[typing.Tuple]:
    ...
    for idx in range(1, len(boundaries)):
        ...
        partitioned_boxes.append((box_lb, box_ub))
...
if __name__ == '__main__':
    ...
    for idx, box in enumerate(partitioned_boxes):
        ...
        print("lb: (" + str(np.around(box[0][0], decimals=2)) + ", " + str(np.around(box[0][1], decimals=2)) + ", " + str(np.around(box[0][2], decimals=2)) + ")")
        print("ub: (" + str(np.around(box[1][0], decimals=2)) + ", " + str(np.around(box[1][1], decimals=2)) + ", " + str(np.around(box[1][2], decimals=2)) + ")")
